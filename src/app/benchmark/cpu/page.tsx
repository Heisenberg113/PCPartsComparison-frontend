...
export default function CpuBenchmarkPage() {
  ...
  return (
    <div>
      <div style={{ marginBottom: '16px', color: 'var(--color-text-muted)', fontSize: '14px' }}>
        {total > 0 && <span>{total} CPU có điểm benchmark</span>}
      </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--color-border)' }}>
                {['#', 'Tên CPU', 'Hãng', 'Điểm PassMark', 'Hiệu năng tương đối', 'Giá thấp nhất'].map((h) => (
                  <th key={h} style={{
                    padding: '10px 14px',
